...
class Player:
    def __init__(self, grid_pos, health):
        ...
        self.image_idle = pygame.transform.scale(pygame.image.load("../assets/sprites/player.png"),
                                                 (GRID_SIZE, GRID_SIZE))
    ...
    def move(self, direction, enemies):
        ...
        if 0 <= next_pos[0] < COLS and 0 <= next_pos[1] < ROWS:
            if grid[next_pos[1]][next_pos[0]] >= 2:
                get_entity_at_pos(next_pos, enemies)
                if self.destroyedWall and not isinstance(get_entity_at_pos(next_pos, enemies), DestructibleWall):
                    points += 150
                    self.destroyedWall = False

                elif isinstance(get_entity_at_pos(next_pos, enemies), DestructibleWall) and not self.destroyedWall:
                    self.destroyedWall = True
...
def fitness_func(ga_instance, solution, solution_idx):
    ...
    while running:
        for move in solution:
            ...
            if not player[0].alive or len(enemies_without_walls) == 0:
                running = False
        running = False
    global points
    enemies_without_walls = list(filter(isNotWall, enemies))
